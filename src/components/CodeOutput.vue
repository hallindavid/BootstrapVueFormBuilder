...
&lt;script&gt;
	export default {
		data: function() {
			return {
				<template
  v-for="(item,index) in text_models"
><span :key="'text_models'+index" v-text="item.model"></span>:"",
				</template><template
  v-for="(item,index) in checkbox_models"
><span :key="'cb_models'+index" v-text="item.model"></span>:false,
				</template><template
  v-for="(item,select_index) in select_models"
><span :key="'select_models'+select_index" v-text="item.model"></span>:null,
				<span v-text="item.model+'_options'" :key="'select_model_option_header_'+select_index"></span>: [
					<template v-for="(option, option_index) in item.select_options"><span
  :key="'select_'+select_index+'_option_'+option_index"
>{ value:"{{ option.value }}", text: "{{ option.text }}" },</span>
					</template>
				],
				</template>
			}
